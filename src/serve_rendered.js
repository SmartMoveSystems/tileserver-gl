...
module.exports = {
  init: (options, repo) => {
    ...
    const respondImage = (item, z, lon, lat, bearing, pitch, width, height, scale, format, res, next, opt_overlay, bounds, opt_mode='tile') => {
    ...
    if (options.serveStaticMaps !== false) {
      ...
      app.get(util.format(staticPattern, centerPattern), (req, res, next) => {
        ...
        return respondImage(item, z, x, y, bearing, pitch, w, h, scale, format, res, next, overlay, bounds, 'static');
      });
